...
$().ready(function() {
	...
	$(".productImageUploadButton input").livequery("change", function() {
		...
		if (/(<#list systemConfig.allowedUploadImageExtension?split(stack.findValue("@com.larkersos.bean.SystemConfig@EXTENSION_SEPARATOR")) as list><#if list_has_next>.${list}|<#else>.${list}</#if></#list>)$/i.test($this.val()) == false) {
			$.message("您选择的文件格式错误！");
			return false;
		}
		...
		<form id="inputForm" class="validate" action="<#if isAdd??>product!save.action<#else>product!update.action</#if>" method="post" enctype="multipart/form-data" >
			...
			<#if (adminDepartment == true)!>
				<tr>
					...
					<td>
						<input type="text" name="product.department" id="Department" class="formText{required: true}" value="${(product.department)!}" readonly/>
						<label class="requireField">*</label>
						<input type="button" class="formButton" onclick="selectDepartmentPage();" value="选   择" hidefocus="true" />
						<input type="hidden" name="product.departmentId" id="DepartmentId" value="${(product.departmentId)!}" />
					</td>
				</tr> 
			</#if>
				<tr>
				...
				<input type="hidden" class="formText" name="product.productSn" value="${(product.productSn)!}" title="若留空则由系统随机生成" />
				...
				<input type="hidden" name="product.isNew" value="false"/>
				<tr>
					<th>
						排序:
					</th>
					<td>
						<input type="text" name="product.store" class="formText {digits: true}" value="${(product.store)!}" title="只允许输入零或正整数" />				 						
					</td>
				</tr>
