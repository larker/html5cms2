...
var editor2 = CKEDITOR.replace('organiseAboutUs'); 
})
</script>
<#if !id??>
	<#assign isAdd = true />
	...
	<div class="body">
		...
		<form id="inputForm" class="validate" action="<#if isAdd??>organise!save.action<#else>organise!update.action</#if>" method="post" enctype="multipart/form-data" >
			...
			<table>
				<tr>
					<td colspan="2">
						<textarea name="organise.aboutUs" id="organiseAboutUs" style="width: 480px; height: 600px;">${(organise.aboutUs)!}</textarea>
					</td>
				</tr>
			</table>
			...
			<table class="inputTable tabContent">
				<tr>
					<td colspan="2">
						<textarea name="organise.description" id="organiseDescription" class="wysiwyg" style="width: 100%; height: 450px;">${(organise.description)!}</textarea>
					</td>
				</tr>
			</table>
