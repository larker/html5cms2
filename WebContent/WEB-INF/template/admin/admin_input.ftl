...
function selectDepartmentPage(){
	...
	if(returnData != null){
	    document.getElementById("adminDepartmentId").value = returnData[0]; 
	    document.getElementById("adminDepartment").value = returnData[1]; 
	} 
}
</script>
</head>
		...
		<form id="inputForm" class="validate" action="<#if isAdd??>admin!save.action<#else>admin!update.action</#if>" method="post">
			...
			<table class="inputTable tabContent">
				<tr>
					...
					<td>
						<input type="text" name="admin.department" id="adminDepartment" class="formText{required: true}" value="${(admin.department)!}" readonly/>
						<label class="requireField">*</label>
						<input type="button" class="formButton" onclick="selectDepartmentPage();" value="选   择" hidefocus="true" />
						<input type="hidden" name="admin.departmentId" id="adminDepartmentId" value="${(admin.departmentId)!}" />
					</td>
				</tr> 
				<tr>
					<th>
			...
			<table class="inputTable tabContent">
				...
				<tr>
					...
					<td>
						<input type="text" name="admin.phone" class="formText" value="${(admin.phone)!}" />
					</td>
				</tr>
			</table>
