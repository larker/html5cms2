 ...
 *	Base JavaScript
 *
 *	http://www.larkersos.com
 *
 *	Copyright (c) 2013 larkersos. All rights reserved. 
 **/
...
function priceCurrencyFormat(price) {
	price = setScale(price, larkersos.priceScale, larkersos.priceRoundType);
	return larkersos.currencySign + price;
}

// 格式化商品价格货币（包含货币单位）
function priceUnitCurrencyFormat(price) {
	price = setScale(price, larkersos.priceScale, larkersos.priceRoundType);
	return larkersos.currencySign + price + larkersos.currencyUnit;
}

// 格式化订单金额货币
function orderCurrencyFormat(price) {
	price = setScale(price, larkersos.orderScale, larkersos.orderRoundType);
	return larkersos.currencySign + price;
}
...
$().ready( function() {
	...
	$("textarea.wysiwyg").tinymce({
		script_url: larkersos.base + "/template/common/tiny_mce/tiny_mce.js",
		language: "zh",
		theme: "advanced",
